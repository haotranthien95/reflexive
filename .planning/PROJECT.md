...
- [ ] The practice loop must run offline **after one successful online Setup visit**, served by the Firestore SDK's own on-device cache — and no outbound request happens on any path *inside* the loop, because both reads are on Setup *(narrowed by D-39/D-36 in Phase 3, which gave the app its first network dependency. The honest consequence: a device that has never been online has an empty cache and **cannot start a session** — it lands on the could-not-load state, not on the empty state. The half of the Phase 1 promise that survives is the one that matters mid-drill: after Start there is no code path from the practice screen to the network at all.)*
